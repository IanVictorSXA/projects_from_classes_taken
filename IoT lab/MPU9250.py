...
class MPU9250:
	...
	def myReadRawSensor(self):
		"""Reading raw values of accelerometer, gyroscope and magnetometer
		   reading raw sensor the same way as readsensor
		"""
		...
		self.RawAccelVals = np.squeeze(self.cfg.transformationMatrix.dot((vals[np.newaxis,:3].T)))
		self.RawGyroVals = np.squeeze(self.cfg.transformationMatrix.dot((vals[np.newaxis,4:7].T)))
		self.RawMagVals = (magHighbits + magData[::2])[-3:]
		self.RawTemp = vals[3]

	def myReadSensor(self):
		"""Reading values of accelerometer, gyroscope and magnetometer 

		The functions finds values by applying caliberation values.
		using myReadRawSensor 

		"""
		self.myReadRawSensor()

		self.AccelVals = (self.RawAccelVals * self.AccelScale - self.AccelBias) * self.Accels
		self.GyroVals = self.RawGyroVals * self.GyroScale - self.GyroBias

		if self.Magtransform is None:
			self.MagVals = (self.RawMagVals * self.MagScale - self.MagBias) * self.Mags
		else:
			self.MagVals = np.matmul(self.RawMagVals * self.MagScale - self.MagBias, self.Magtransform)

		self.Temp = (self.RawTemp - self.cfg.TempOffset) / self.cfg.TempScale + self.cfg.TempOffset
