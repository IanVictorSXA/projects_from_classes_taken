...
REG_MAX_SAMPLES = 30             # ✅ always collect 30 samples
SMOOTH_WINDOW = 5
MARGIN = 0.15
...
def detect_faces_and_landmarks(frame_bgr):
    ...
    centers = []
    for f in mesh_faces:
        xs = [lm.x for lm in f.landmark]; ys = [lm.y for lm in f.landmark]
...
# ==================================================
# Register Mode — wait, then collect 30 samples
# ==================================================
mode = input("Select mode: [r]egister or [f]ace recognition? ").strip().lower()
db = load_database()
...
if mode == "r":
    ...
    while len(samples) < REG_MAX_SAMPLES:
        ...
        if crop.size == 0:
            continue
        if best["five"] is not None:
            try:
                aligned = align_face(frame, best["five"])
            except Exception:
    ...
    try:
        while True:
            ...
            if db:
                ...
                if sim >= THRESHOLD:
                    label = name
            print(f"🧍 {label} — cos={score:.3f} — FPS={fps:.1f}")
    except KeyboardInterrupt:
        print("🛑 Exiting.")
        unlock_auto_controls()
        cam.stop()
else:
    print("❌ Invalid mode selected. Use 'r' for register or 'f' for recognition.")
